...
class getFeatures():
    def __init__(self, openface2Path):
        current_path = os.path.abspath('./')
        # working_dir = os.path.abspath(os.path.dirname(current_path) + os.path.sep + ".")
        working_dir = '/data1/yinjiazi/mmsa'
        self.data_dir = os.path.join(working_dir, 'Processed')
        self.label_path = os.path.join(self.data_dir, 'Video/OpenFace2/1/1025/1025.hog')
        # padding
    ...
    def read_hog(self, filename, batch_size=5000):
        """
    ...
    def __getTextEmbedding(self, text):
        ...
        pretrained_weights = '/home/xjnu1/mmsa/bert/bert-base-chinese'
    ...
    def handleImages(self):
        ...
        for image_dir in tqdm(image_dirs):  # /data1/yinjiazi/mmsa/Processed/video/AlignedFaces/Raw/*
            print('imgae_dir:' + image_dir)
            output_dir = image_dir.replace('AlignedFaces', 'OpenFace2')
            print('output_dir:' + output_dir)
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)

            item_dir = os.path.join(image_dir, image_dir + '0001.jpg')
            if not os.path.exists(item_dir):
                cmd1 = 'cp /data1/yinjiazi/mmsa/Processed/video/AlignedFaces/Raw/6_392/0001.png ' + image_dir
                os.system(cmd1)
            ...
            cmd = '/home/admin123/OpenFace/build/bin/FeatureExtraction -fdir ' + image_dir + ' -out_dir ' + output_dir
            # cmd = '/OpenFace/build/bin/FaceLandmarkImg -fdir ' + image_dir + ' -out_dir ' + output_dir
            os.system(cmd)
            print('----- cmd ok! -----')
            # get feature
            _, file_name = os.path.split(output_dir)
            print(type(file_name), file_name)
            csv_path = os.path.join(output_dir, file_name + ".csv")
            print(csv_path)
            df = pd.read_csv(csv_path)
            embedding_V = np.array(df)
            dict_V[file_name] = embedding_V
            # features_V.append(embedding_V)
            # remove  file
            # shutil.rmtree(output_dir)
        # save
        import pickle
        with open('openface.p', 'wb') as f:
    ...
    def get_video_feature(self):
        ...
        dir_path = os.path.join(self.data_dir, 'video\\OpenFace2\\1')
        dir_list = os.listdir(dir_path)

        for i in tqdm(range(len(dir_list))):
            csv_path = os.path.join(dir_path, dir_list[i], dir_list[i] + ".csv")
            df = pd.read_csv(csv_path)
            embedding_V = np.array(df)
            features_V.append(embedding_V)
    ...
    def pad_audio_and_video(self):
        from utils.functions import read_pickle, write_pickle
        import joblib

        # clf = joblib.load('filename.pkl')
        import gc
        def pad_and_save(audio, path):
            data = []
            keys = []
            data_dict = {}
            for k, v in audio.items():
                ...
                keys.append(k)
            pad_audio = self.__paddingSequence(data)

            for key, value in zip(keys, pad_audio):
                data_dict[key] = value

            del data, keys, pad_audio
            gc.collect()

            joblib.dump(data_dict, path, compress=3)

            # write_pickle(path,data_dict)

        # audio = read_pickle('../feature_extractor/audio/audio.p')
        # pad_and_save(audio, path='../feature_extractor/audio/audio_pad.p')

        vision = read_pickle('../feature_extractor/video/video_resnet.p')
        pad_and_save(vision, path='../feature_extractor/video/video_pad.p')
...
if __name__ == "__main__":
    ...
    gf = getFeatures(args.openface2Path)

    # using openface2 to extract face landmark
    gf.handleImages()
    # print('--' * 50)
    # os.system('shutdown')
    # gf.pad_audio_and_video()
    # gf.get_video_feature()
    # gf.results('features')
